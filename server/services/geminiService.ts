  ...
  private buildRepositoryChatPrompt(
    ...
    if (context) {
      ...
      if (context.files?.length) {
        prompt += `Files: ${context.files.slice(0, 10).join(", ")}${
      ...
      if (context.contributors?.length) {
        prompt += `Contributors: ${context.contributors
          .slice(0, 5)
          .join(", ")}\n`;
      }
      prompt += "\n";
...
let cachedGeminiService: GeminiService | undefined;

function getGeminiService(): GeminiService {
  if (cachedGeminiService) return cachedGeminiService;

  // Constructing GeminiService throws if the key is missing; we do that lazily
  // so the server can still boot and serve non-AI routes.
  cachedGeminiService = new GeminiService();
  return cachedGeminiService;
}

// Export a lazy proxy so existing imports (`geminiService.*`) keep working.
export const geminiService = new Proxy({} as GeminiService, {
  get(_target, prop: string | symbol) {
    const service: any = getGeminiService();
